...
<template>
  <header class="sticky top-0 z-40 border-b border-white/10 bg-black/80 backdrop-blur">
    <nav class="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8" aria-label="Primary">
      <NuxtLink to="/" class="flex items-center gap-2 text-lg font-semibold tracking-tight text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-500 rounded-md">
      ...
      >
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
          <path v-if="!open" stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5M3.75 17.25h16.5" />
    ...
    <div v-if="open" id="mobile-nav" class="border-t border-white/10 px-4 py-4 md:hidden">
      ...
      <div class="mt-3 flex flex-col gap-2 border-t border-white/10 pt-3">
        <NuxtLink to="/login" class="rounded-lg px-3 py-3 text-center text-sm font-semibold text-slate-200 hover:bg-white/5" @click="open = false">Log in</NuxtLink>
        <NuxtLink to="/signup" class="rounded-lg bg-rose-600 px-3 py-3 text-center text-sm font-semibold text-white" @click="open = false">Start streaming</NuxtLink>
      </div>
    </div>
  </header>
</template>
